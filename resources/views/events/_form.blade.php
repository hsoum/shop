...
@if (Auth::user()->hasRole('admin') && $event->user_id)
...
<div class="form-group row">
    ...
    <div class="col-md-8">
        <input id="planned_on" type="date" class="form-control {{ $errors->has('planned_on') ? ' is-invalid' : '' }}"
            name="planned_on" value="{{ old('planned_on', optional($event->planned_on)->format('Y-m-d')) }}" required
            autofocus>
    </div>
</div>
...
<div class="form-group row">
    ...
    <div class="col-md-8">
        <textarea rows="4" id="description" type="text"
            class="form-control {{ $errors->has('description') ? ' is-invalid' : '' }}" name="description" required
            autofocus>{{ old('description', $event->description) }}</textarea>
    </div>
</div>
<div class="form-group row">
    ...
    <div class="custom-control custom-checkbox align-self-center col-md-4">
        <input id="status" type="checkbox"
            class="custom-control-input {{ $errors->has('status') ? ' is-invalid' : '' }}" name="status"
...
@if (Route::currentRouteName() === 'events.edit' && (Auth::user()->hasRole('admin')|| (Auth::user()->hasRole('It'))))
