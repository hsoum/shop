...
<div class="container">
    ...
    <table class="table table-dark table-striped table-hover mt-4">
        ...
        <tbody>
            ...
            <tr>
                <td style="vertical-align: middle"><a href="{{ route('events.show', compact('event')) }}"
                        class="text-white event-show-link">{{ $event->name }}</a></td>
                <td style="vertical-align: middle; text-align: justify">
                    {{ str_limit($event->description, 160, ' (...)') }}</td>
        ...
        <div class="card comment">
            <div class="card-body">
                ...
                <p class="card-subtitle mb-2 text-muted">
                    {{ $comment->user->campus }} &middot; <a
                        href="{{ route('events.show', ['id' => $comment->commentable->id]) }}">{{ $comment->commentable->name }}</a>
                </p>
            </div>
        </div>
        @endforeach
